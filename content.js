...
(async () => {
    try {
        ...
        // 动态导入所有模块
        const { StorageManager } = await import(baseUrl + 'StorageManager.js');
        const { SelectionTracker } = await import(baseUrl + 'SelectionTracker.js');
        const { QuestionQueueManager } = await import(baseUrl + 'QuestionQueueManager.js');
        const { ChatGPTDetector } = await import(baseUrl + 'ChatGPTDetector.js');
        const { AutoSender } = await import(baseUrl + 'AutoSender.js');
        const { UIController } = await import(baseUrl + 'UIController.js');
        ...
        console.error('[Content] 加载模块失败:', error);
    }
})();
